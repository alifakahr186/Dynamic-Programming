...
long long DiceRollsTable(int d, int faces, int t) {
    std::vector<long long> prev(t + 1, 0);
    std::vector<long long> curr(t + 1, 0);

    prev[0] = 1;

    for (long long dice = 1; dice <= d; dice++) {
        for (long long target = 0; target <= t; target++) {
            curr[target] = 0; // Reset the current target value in the current row
            for (long long i = 1; i <= faces; i++) {
                if (target - i >= 0) {
                    curr[target] += prev[target - i];
                }
            }
        }
